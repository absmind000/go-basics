...
func main() {
	...
	// スライスへの要素追加
	s1 = append(s1, 1, 2, 3) // 複数要素を追加　appendは末尾に要素追加
	fmt.Printf("s1: %[1]T %[1]v %v %v\n", s1, len(s1), cap(s1))
	// 出力: s1: []int [1 2 3] 3 3
	...
	s6[1] = 10    // s6の変更はs5にも影響する（同じ配列を参照）s6の2番目の要素に代入している
	fmt.Printf("s5: %v %v %v\n", s5, len(s5), cap(s5))
	// 出力: s5: [0 0 10 0] 4 6
	...
	fmt.Printf("sc6 dst of copy after: %v %v %v\n", sc6, len(sc6), cap(sc6)) // 新しいスライスの要素は自動的に0で初期化される
	...
	fmt.Printf("fs6: %v %v %v\n", fs6, len(fs6), cap(fs6)) // 容量は2倍ずつ増えるGoの仕様
	// 出力: fs6: [6 7 8] 3 4

	// マップの宣言と操作
	var m1 map[string]int  // nilマップの宣言
	m2 := map[string]int{} // 空のマップの初期化、nilではない
	fmt.Printf("%v %v \n", m1, m1 == nil)
	// 出力: map[] true
